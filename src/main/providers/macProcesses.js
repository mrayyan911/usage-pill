...
// like `-p "hello world"` becomes two tokens once ps merges argv into one
// string — an accepted, documented limitation (see processSessions.js's
// classifyProcess: VALUE_OPTIONS only ever skips exactly one token after a
// flag, so at most the second half of such a value could be misread).
...
// Pure: parses `ps -eo pid,ppid,lstart,command` text output (header row
// included) into the normalized row shape shared with the Windows/Linux
// providers.
...
function readMacProcesses({ signal } = {}) {
  ...
  return new Promise((resolve, reject) => {
    execFile('ps', ['-eo', 'pid,ppid,lstart,command', '-U', String(uid)], {
